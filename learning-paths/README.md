...
| 08 | [Mathematical Foundations](./08-mathematical-foundations/) | Anyone who wants the theory behind the engineering | 🟢 → 🔴 (mixed) | ✅ v1 COMPLETE — all 13 math pages authored (Batches 53 + 67); 🔧 Batch 68 refresh — every page rewritten with Python code examples, "Common mistakes" sections, and clean GitHub-rendered LaTeX |
| 09 | [Capstones](./09-capstones/) | Anyone ready to build something portfolio-worthy | 🔴 Advanced | ✅ v1 COMPLETE — all 8 project briefs shipped (Batches 62-65) |
...
| 06 Evaluation & Observability | ☑ | ● | ● | ● | ● | ● |
| 07 Production & Safety | ☑ | ☐ | ☐ | ☐ | ☐ | ☐ |
| 08 Mathematical Foundations | ☑ | ● (13 of 13) | n/a | n/a | n/a | n/a |
| 09 Capstones | ☑ | n/a | n/a | n/a | n/a | ☐ |

Checkbox status is updated at each release. **All nine paths now have authored landing READMEs as of Batch 42; Path 04 is complete — all 7 modules shipped (the MCP build-consume-secure trio + A2A foundations + A2A production depth + MCP+A2A composition) as of Batch 50; the top-level [`patterns/`](../patterns/) catalog is complete — all 12 pages authored (Patterns 01-12) as of Batch 52; Path 03 v3 capstone projects are complete — three production-deployable projects (customer-support, research pipeline, A2A-federated) shipped as of Batch 53; Path 03 is now fully symmetric with Path 06's structure — the multi-agent frameworks deep dive (9 frameworks compared) shipped as of Batch 54; Path 07 v1 opened in Batch 55 — Modules 1 + 8 (deployment patterns + pre-launch checklist) shipped as the bookend modules; Path 07 v1 continued through Batches 56-58 (4 + 4 modules), with Path 07 v1 COMPLETED in Batch 58 (all 8 of 8 modules); Path 05 v1 opened in Batch 59 (Modules 1 + 2), continued in Batch 60 (Modules 3 + 4), and COMPLETED in Batch 61 (Modules 5 + 6) — all 6 of 6 modules; Path 09 v1 opened in Batch 62 with the bookend pair (Beginner #01 + Capstone #07), opening the `projects/beginner/` and `projects/capstone/` tier subdirs; continued in Batch 63 (Beginner #02 + Intermediate #03), opening `projects/intermediate/`; continued in Batch 64 (Intermediate #04 + #05) completing the Intermediate tier; Batch 65 closed Path 09 v1 with the final capstone pair (Projects #06 + #08); Batch 66 opened the `glossary/` folder with `terms.md` (~65 A-Z entries) at [`glossary/terms.md`](../glossary/terms.md), with cross-references added to `concepts/`, `patterns/`, and `math-foundations/` landing pages; **Batch 67 COMPLETED Path 08 v1 — the 9 remaining math pages (01 LM probability, 02 embeddings, 03 RAG formulation, 05 MDP/POMDP, 07 tool selection, 08 planning, 09 memory, 10 multi-agent coordination, 11 evaluation metrics, 12 uncertainty + safety, 13 context-window optimization) shipped at [`math-foundations/`](../math-foundations/), bringing the path from 4/13 to 13/13; together with the [`learning-paths/08-mathematical-foundations/README.md`](./08-mathematical-foundations/) upgrade to v1-complete framing, Path 08 is now the ninth and final v1-complete path; the glossary's math entries (Action, MDP, POMDP, Belief state, Policy, Embedding) updated to live links to the new math pages**. 🎉 **All nine paths now have substantial content. The repo's v1 phase is structurally complete** — five paths fully complete at v1 level (Paths 04, 05, 07, 08, 09) plus Path 01 complete, plus Paths 02 v1+v2 / 03 v1+v2+v3+frameworks / 06 v1+v2 as substantial-content paths. Future work is continuous improvement (depth, breadth, examples, recipes, references) rather than gap-filling. If you want to help fill in a row, [`CONTRIBUTING.md`](../CONTRIBUTING.md) walks you through the workflow for each content type.
