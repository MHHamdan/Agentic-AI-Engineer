...
> 🟡 Intermediate · ⏱ 6–10 hours (planned, ~7 hours shipped via Modules 1+2+3+4+5+6+7) · 📍 Start here once you've completed Path 01 · ✅ **Complete — all 7 modules shipped (Batches 43-50)**

> ✅ **All seven modules are live.** Seven concept pages ([MCP foundations](../../concepts/tools/mcp-foundations.md) + [Building an MCP server](../../concepts/tools/building-an-mcp-server.md) + [Building an MCP client](../../concepts/tools/building-an-mcp-client.md) + [MCP security threat model](../../concepts/tools/mcp-security-threat-model.md) + [A2A foundations](../../concepts/tools/a2a-foundations.md) + [A2A endpoint at production depth](../../concepts/tools/a2a-endpoint-production-depth.md) + [MCP + A2A composition](../../concepts/tools/mcp-a2a-composition.md)), six labs ([Lab 25](../../labs/25-mcp-server-from-scratch/) + [Lab 26](../../labs/26-mcp-client-from-scratch/) + [Lab 27](../../labs/27-mcp-security-threat-model/) + [Lab 28](../../labs/28-a2a-endpoint-from-scratch/) + [Lab 29](../../labs/29-a2a-endpoint-production-depth/) + [Lab 30](../../labs/30-mcp-a2a-composition/)), and six quizzes ([MCP foundations and server](../../quizzes/foundations/mcp-foundations-and-server.md) + [MCP client and discovery](../../quizzes/foundations/mcp-client-and-discovery.md) + [MCP security threat model](../../quizzes/foundations/mcp-security-threat-model.md) + [A2A foundations](../../quizzes/foundations/a2a-foundations.md) + [A2A endpoint production-depth](../../quizzes/foundations/a2a-endpoint-production-depth.md) + [MCP + A2A composition](../../quizzes/foundations/mcp-a2a-composition.md)) ship as of Batch 50. Path 04 is complete; the "What you can read right now" section lists every shipped artifact plus adjacent existing material.
...
| 7 | **[MCP + A2A composition](../../concepts/tools/mcp-a2a-composition.md)** — the asymmetric pattern: MCP for agent-to-tool internally, A2A for agent-to-agent externally; the canonical 40-60% workflow-velocity improvement framing per a2a-mcp.org March 2026; client surface via `A2ACardResolver` + `ClientFactory` + `ClientConfig`; push notifications with atomic registration via `SendMessageConfiguration.task_push_notification_config` + `return_immediately=True` (the deferral from Module 6 closes here); the `X-A2A-Notification-Token` shared-secret pattern; idempotency requirements for webhook receivers; the four limits composition does NOT solve (capability inference, multi-hop tracing, cross-protocol type system, cross-protocol backpressure). Lab: [Lab 30 — MCP + A2A composition](../../labs/30-mcp-a2a-composition/). Quiz: [MCP + A2A composition](../../quizzes/foundations/mcp-a2a-composition.md) | ✅ Shipped (Batch 50) |
...
The repo already has substantive material that maps to this path. All seven modules are now live; the artifacts below include the shipped module content plus adjacent existing material the path builds on:

**Shipped — Modules 1+2+3+4+5+6+7** (Batches 43 + 46 + 47 + 48 + 49 + 50):
...
- 📖 [MCP + A2A composition](../../concepts/tools/mcp-a2a-composition.md) — Module 7 concept page (~14 min). The closer for Path 04. The asymmetric composition shape (MCP for agent-to-tool internally, A2A for agent-to-agent externally); when the composition is right vs overkill; the composed-worker code shape (~30 lines on top of Lab 28's hello agent: `MCPClient` opened inside `execute()`, two tool calls inside one MCP client lifetime, result packaged as A2A artifact); the orchestrator code shape via `A2ACardResolver` + `ClientFactory(config=ClientConfig(...))`; push notifications with atomic registration via `SendMessageConfiguration.task_push_notification_config` + `return_immediately=True` (the deferral from Module 6 closes here); the `X-A2A-Notification-Token` shared-secret pattern; signature verification wired into `ClientFactory.create_from_url` via the `signature_verifier` callback; the four limits composition does NOT solve (capability inference, native multi-hop tracing, cross-protocol type system, cross-protocol backpressure). Path 04 closure framing: OAuth2 cross-org token flows + distributed tracing across the composition are explicitly future work.
...
- 🧪 [Lab 30 — MCP + A2A composition](../../labs/30-mcp-a2a-composition/) — 130-160 min intermediate lab; closer for Path 04. Three subprocesses driven from one notebook: an MCP server (FastMCP 3.3, two tools — `lookup_customer` + `summarize_request`), an A2A worker whose `execute()` opens an `MCPClient` inside the request lifetime (so MCP stays an internal implementation detail), and a tiny Starlette webhook receiver capturing push notifications to a JSONL file. Orchestrator-side flow demonstrates `A2ACardResolver` → `ClientFactory(config=ClientConfig(streaming=False)).create(card)` → `client.send_message(SendMessageRequest(message=msg))`. Push notifications via atomic registration: `SendMessageConfiguration(task_push_notification_config=..., return_immediately=True)`; webhook captures 2-4 callbacks per task (artifact + status transitions); `X-A2A-Notification-Token` carries the shared secret. Encapsulation check: the worker's Agent Card describes the *capability* but doesn't mention MCP anywhere — orchestrators get the contract, never the implementation.
...
- 🧠 [MCP + A2A composition quiz](../../quizzes/foundations/mcp-a2a-composition.md) — 8 single-select questions covering Module 7 + Lab 30 (asymmetric composition, atomic push registration, `return_immediately` semantics, `X-A2A-Notification-Token`, idempotency requirements, the four limits composition does NOT solve); pass at 6/8.
...
- [Pattern 12 — A2A federation](../../patterns/12-a2a-federation.md) — the architecture-level companion to Modules 5+6+7; cross-agent task delegation via the Agent-to-Agent protocol. Authored in Batch 44; Modules 5+6+7 now operationalize it (foundations + production depth + composition).
...
Path 04 is complete — all seven modules shipped across Batches 43, 46, 47, 48, 49, and 50. Two production concerns remain explicitly future work; both are large enough to belong to other paths or major batches:

- **OAuth2 cross-org token flows** — Module 6 sketched the security scheme; Module 7 wired in signature verification at the client side; full integration with a real auth issuer (token issuance, JWKS distribution, token rotation) needs an auth server in the lab environment and naturally fits a future Path 08 (Production Engineering) module.
- **Distributed tracing across the composition** — `concepts/evaluation/opentelemetry-genai-conventions.md` covers the GenAI semantic conventions. Composition-specific work — `traceparent` header forwarding across the A2A boundary, span linking from orchestrator → worker → MCP server — belongs in observability tooling. The SDK doesn't automate this yet.

Contributions are welcome. The way to extend Path 04:

1. **Open an issue** describing the production concern you want to operationalize (cross-org OAuth2, distributed tracing, webhook hardening, multi-worker fan-out).
2. **Read [`CONTRIBUTING.md`](../../CONTRIBUTING.md)** — the source-citation rules and per-content-type templates are non-negotiable.
3. **Pick one concern at a time.** Each Path 04 module is one concept page + one lab + one quiz; future contributions should fit that shape.
