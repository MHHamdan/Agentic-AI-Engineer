...
Plus the [recipes index](./recipes/README.md) with the choose-your-recipe decision tree, and the [`_template.md`](./recipes/_template.md) for adding future recipes.

**What the recipes give you that the v1 modules don't**: each recipe maps Path 06 modules to concrete production actions for a specific deployment shape, contrasts lab-shape vs production-shape per module, documents the hand-off points between artifacts, and includes a cost envelope at three traffic scales (10K / 100K / 1M traces/month). Recipe 3's hand-off discipline section is the central artifact for hybrid deployments.

### Patterns (Batch 34) — reusable cross-cutting mechanisms

If recipes are end-to-end compositions, **patterns are the mechanisms inside them**. A team running the Hybrid recipe doesn't just install LangSmith + OTel — they also need cost-aware retrieval, drift-triggered review, and judge-ensemble combination strategies.

| # | Pattern | Solves | Module anchor |
|---|---------|--------|---------------|
| 1 | [Cost-aware retrieval](./patterns/01-cost-aware-retrieval.md) | Retrieval cost explodes when k, reranking, and agentic loops fire on every query | M6 cost attribution · adaptive sampling |
| 2 | [Drift-triggered review](./patterns/02-drift-triggered-review.md) | Auto-retrain on drift is the wrong default; drift signals route traces to a human review queue | M5 drift detection · judge calibration |
...
### Projects (Batch 35) — production-deployable capstones
...
|---|---------|-----------|----------|
| 1 | [LangSmith eval stack](./projects/01-langsmith-eval-stack.md) | LangChain-rooted teams; fastest zero-to-production eval workflow (~3-5 day build) | Recipe 1 + Patterns 2 & 3, Labs 17/19/20/22 |
| 2 | [OTel observability stack](./projects/02-otel-observability-stack.md) | Teams with existing observability stack; vendor-neutral telemetry (~4-6 day build) | Recipe 2 + Patterns 1/2/3, Labs 18/19/20/21 |
| 3 | [Hybrid production stack](./projects/03-hybrid-production-stack.md) | Production teams needing both LLM-eval UX **and** vendor-neutral telemetry — **the central capstone** (~5-8 day build) | Recipe 3 + **all three patterns**, **all seven v1 modules** |

Plus the [projects index](./projects/README.md) with the pick-a-project decision tree, and the [`_template.md`](./projects/_template.md) for adding future projects.
...
| `projects/README.md` (projects index) | ✅ shipped (batch 35) |
| `projects/01-langsmith-eval-stack.md` | ✅ shipped (batch 35) |
| `projects/02-otel-observability-stack.md` | ✅ shipped (batch 35) |
