...
> 🟡 Intermediate · ⏱ 4–8 hours · 📍 Start here after Path 01 + one of Path 02 or Path 03 · ✅ **v1 complete** — all 6 modules shipped (Batches 59-61)

> ✅ **v1 complete — all 6 modules shipped.** Modules 1 + 2 (Context engineering foundations + Token budgets per zone) shipped in Batch 59 as the vocabulary-establishing opening pair. Modules 3 + 4 (Compression and summarization + Memory tiers) shipped in Batch 60 as the operational layer — compression makes the soft-cap pattern from Module 2 enforceable; memory tiers extends the architecture across sessions. Modules 5 + 6 (Context drift detection + Long-context models — when they help and when they don't) shipped in Batch 61 as the closing pair — drift detection surfaces when the architecture isn't working; long-context model selection covers the design decision the path had been deferring. Path 05 v1 is now structurally complete; Path 05 v2 (deeper lab content, framework-specific patterns, additional model-landscape detail) would be a future expansion.
...
| 5 | **Context drift detection** — the four early-warning signals (re-reads, re-decisions, task reframing, retrieval-precision collapse); instrumenting at trace level | ✅ Shipped (Batch 61) — [`concepts/context/context-drift-detection.md`](../../concepts/context/context-drift-detection.md) |
| 6 | **Long-context models — when they help and when they don't** — the 1M-token tier (Claude Sonnet 4 beta, MiniMax-M1, Qwen3); pricing tier cliffs; the "needle in haystack" performance degradation; choosing context size as a design decision | ✅ Shipped (Batch 61) — [`concepts/context/long-context-models.md`](../../concepts/context/long-context-models.md) |
...
The first batch (Modules 1 + 2 — foundations + token budgets per zone) shipped in Batch 59 as the opening pair. The second batch (Modules 3 + 4 — compression and summarization + memory tiers) shipped in Batch 60 as the operational layer. The closing batch (Modules 5 + 6 — context drift detection + long-context models) shipped in Batch 61, bringing Path 05 v1 to 6 of 6 modules and closing a clean three-batch arc (59 + 60 + 61). Path 05 is now the third path complete at v1 level, after Path 04 (Batch 50) and Path 07 v1 (Batch 58). Future work falls into Path 05 v2 territory: deeper lab content with executable notebooks for context-budget enforcement, framework-specific walkthroughs (LangGraph checkpointer patterns, Mem0 integration recipes, Anthropic compaction API recipes), and additional model-landscape detail as the 1M-token frontier-model space evolves.
