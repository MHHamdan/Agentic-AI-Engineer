...
**Format.** Each question shows four options. Open the `<details>` block to reveal the answer and explanation, with a pointer back to the exact concept-page section. Aim for 6/8; below that, the linked section is the place to revisit.

## Q1 — Step zero, with no eval set

Your RAG system has no eval set yet, but the median query looks fine in spot checks. According to the decision guide, what is the correct next step before adopting any SOTA pattern?
...
Review: [`concepts/rag/sota-rag-patterns.md` § Choosing a pattern](../../concepts/rag/sota-rag-patterns.md#choosing-a-pattern)
</details>

## Q2 — The cheapest pattern to adopt

Which SOTA pattern is described as among the cheapest to adopt because it adds a single classification step (a retrieval evaluator) rather than a full extra retrieval loop?
...
Review: [`concepts/rag/sota-rag-patterns.md` § Pattern 2: Corrective RAG (CRAG)](../../concepts/rag/sota-rag-patterns.md#pattern-2-corrective-rag-crag)
</details>

## Q3 — A wide spread of query difficulty

A workload has a wide spread of query difficulty: many trivial lookups and a few genuinely multi-hop questions. Applying multi-step retrieval to every query wastes budget; applying single-step to every query fails the hard ones. Which pattern targets exactly this?
...
Review: [`concepts/rag/sota-rag-patterns.md` § Pattern 3: Adaptive RAG (query-complexity routing)](../../concepts/rag/sota-rag-patterns.md#pattern-3-adaptive-rag-query-complexity-routing)
</details>

## Q4 — Self-RAG's reflection tokens

Self-RAG introduces special tokens the model emits to make retrieval and quality decisions explicit. What do the IsREL, IsSUP, and IsUSE reflection tokens correspond to?
...
Review: [`concepts/rag/sota-rag-patterns.md` § Pattern 1: Self-RAG (self-reflective retrieval)](../../concepts/rag/sota-rag-patterns.md#pattern-1-self-rag-self-reflective-retrieval)
</details>

## Q5 — When Graph RAG earns its index cost

For which query type is Graph RAG specifically strong, justifying its high index-time cost?
...
Review: [`concepts/rag/sota-rag-patterns.md` § Pattern 5: Graph RAG](../../concepts/rag/sota-rag-patterns.md#pattern-5-graph-rag)
</details>

## Q6 — The long-context temptation

A teammate proposes switching to a 1M-token context model and dropping fine-grained retrieval, expecting strictly better answers because 'more context cannot hurt.' What is the most accurate caution from the long-context RAG material?
...
Review: [`concepts/rag/sota-rag-patterns.md` § Pattern 6: Long-context RAG](../../concepts/rag/sota-rag-patterns.md#pattern-6-long-context-rag)
</details>

## Q7 — Multi-hop evidence, spread across documents

Multi-hop questions are failing: the evidence to answer is spread across several documents and a single retrieval never surfaces all of it. Which pattern is the targeted fix, and what is its main cost?
...
Review: [`concepts/rag/sota-rag-patterns.md` § Pattern 4: Agentic RAG](../../concepts/rag/sota-rag-patterns.md#pattern-4-agentic-rag)
</details>

## Q8 — The pattern still flagged as emerging

The page marks one pattern's tooling and benchmarks as still developing, advising you to verify model recommendations against current benchmarks and budget for manual validation. Which pattern is flagged this way?
...
Review: [`concepts/rag/sota-rag-patterns.md` § Pattern 7: Multimodal RAG](../../concepts/rag/sota-rag-patterns.md#pattern-7-multimodal-rag)
</details>
...
|---|---|
| 8/8 | Solid — you can match a pattern to a measured failure and name what its loop costs. |
| 6-7/8 | Working knowledge — re-skim the explanation on each miss. |
| 4-5/8 | Re-read the pattern sections you missed, then re-attempt. |
| ≤3/8 | Read [`concepts/rag/sota-rag-patterns.md`](../../concepts/rag/sota-rag-patterns.md) end to end before re-attempting. |

## Next

- 🧠 [`concepts/rag/sota-rag-patterns.md`](../../concepts/rag/sota-rag-patterns.md) — the reading this quiz checks; the *Choosing a pattern* section is the decision guide.
- 🧪 [Lab 34 — RAG pattern head-to-head](../../labs/34-rag-pattern-head-to-head/) and [Lab 35 — adaptive router](../../labs/35-adaptive-rag-router/) — where you measure these patterns against each other instead of arguing about them.
- 📊 [RAG evaluation quiz](./rag-evaluation.md) — the eval-set discipline that makes "adopt in response to measured failures" actionable.
