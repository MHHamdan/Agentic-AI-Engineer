"""Runnable lab demo for Stateful vs Stateless."""
...
FOCUS = "externalize session state so requests can move between instances"
BASELINE_STRATEGY = "sticky_stateful_instance"
IMPROVED_STRATEGY = "stateless_with_shared_store"
PRIMARY_METRIC = "failed_after_restart"
...
def compare_strategies() -> dict[str, ScenarioResult]:
    ...
    baseline = ScenarioResult(
        ...
        metrics={PRIMARY_METRIC: 3, "operations": 10},
    )
    improved = ScenarioResult(
        strategy=IMPROVED_STRATEGY,
        metrics={PRIMARY_METRIC: 0, "operations": 10},
    )
